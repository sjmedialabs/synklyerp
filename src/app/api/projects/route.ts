...
export async function GET(req: Request) {
  try {
    ...
    const result = await repo.listProjects(tenantId, { ...params, status });
...
export async function POST(req: Request) {
  try {
    const { tenantId } = await getTenantApiContext(P.projects.projects.create, { req });
    const body = projectSchema.parse(await req.json());
    return apiSuccess(await repo.createProject(tenantId, body), undefined, 201);
  } catch (error) {
