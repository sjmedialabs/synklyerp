...
import { createWebhook, listWebhooks } from "@/repositories/sales/crm/forms-webhooks";
...
export async function GET(req: Request) {
  try {
    ...
    const result = await listWebhooks(tenantId, params);
...
export async function POST(req: Request) {
  try {
    ...
    const body = crmWebhookSchema.parse(await req.json());
    return apiSuccess(await createWebhook(ctx.tenantId, ctx.userId, body), undefined, 201);
  } catch (error) {
