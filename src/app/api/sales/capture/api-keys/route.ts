...
import * as repo from "@/repositories/sales/crm/api-keys";
import { crmApiKeySchema } from "@/validators/crm";
import { z } from "zod";
...
export async function GET(req: Request) {
  try {
    ...
    const result = await repo.listApiKeys(tenantId, params);
...
export async function POST(req: Request) {
  try {
    ...
    const body = crmApiKeySchema.parse(await req.json());
    return apiSuccess(await repo.createApiKey(ctx.tenantId, ctx.userId, body), undefined, 201);
  } catch (error) {
